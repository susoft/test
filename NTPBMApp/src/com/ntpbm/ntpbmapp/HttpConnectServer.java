...
public class HttpConnectServer {
	...
	//Tomcat 서버 호출 nameValuePairs 방식.. 호출.
	public StringBuffer sendByHttpPair(StringBuffer strbuf, String urlStr) {
		...
		try {
			...
			String[] temp = strbuf.toString().split("&");
			for (int i = 0 ; i < temp.length; i++) {
				String[] temp1 = temp[i].split("=");
				
				if (temp1.length < 2) nameValuePairs.add(new BasicNameValuePair(temp1[0], ""));
				else nameValuePairs.add(new BasicNameValuePair(temp1[0], temp1[1]));
			}
	...
	//서버에서 받은 json 결과를 hashmap에 넣어 리턴한다.
	public List<HashMap<String, String>> jsonParserArrayList(String pRecvServerPage, String[] jsonName) {
		...
		try {
			...
			for (int i = 0; i < jArr.length(); i++) {
				...
				if (json != null) {
					...
					for (int j = 0; j < jsonName.length; j++) {
						parseredData.put(jsonName[j], checkNull(json.getString(jsonName[j])));
						
						Log.i("test:", json.getString(jsonName[j]));
					}
					
					parseredDataList.add(i,parseredData);
				}
			}
